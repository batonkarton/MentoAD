...
public class MotorcycleUpdateService : MotorcycleShowInfo
{
    ...
    public void Update(MotorcycleRepository repository)
    {
        while (true)
        {
            ...
            if (inputAnswer.Key == ConsoleKey.Y)
            {
                ...
                switch (inputUpdate)
                {
                    case "brand":
                        ...
                        break;
                    case "year":
                        if (int.TryParse(newValue, out var newYear)) selectedMotorcycle.Year = newYear;
                        Log.Information("Year updated");
                        break;
                    case "mileage":
                        if (int.TryParse(newValue, out var newMileage)) selectedMotorcycle.Mileage = newMileage;
            ...
            {
                Log.Warning("Invalid input ,must be 'y' or 'n'");
                Console.WriteLine($"Invalid input {inputAnswer}");
                continue;
            }
